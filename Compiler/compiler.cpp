...
void syntaxerdriver(std::string filename) {
    ...
    int index = 0, while_addr;
    TDPPstack.push("<RAT17F>");
    Token i = inputstring[index];
    std::cout << "Token: " << i.token << " Lexeme: " << i.lexeme << std::endl;
    bool assign_flag = 0, while_flag = 0, if_flag = 0, cond_flag = 0, write_flag = 0, skip = 0;
    ...
    while (TDPPstack.top() != "$") {
        ...
        if (!isnonterminal(t)) {
            if (t == i.lexeme || t == i.token) {
                if (i.token == "id" && assign_flag) {
                    if (inputstring[index + 1].lexeme == ":=") {
                        ...
                        if (!skip) {
                            ...
                            skip = false;                            
                        std::string next = inputstring[index + 1].lexeme;
                ...
                else if (i.token == "id" && (while_flag || if_flag)) {
                    int sym_index = symbol_table_lookup(i);
                    if (sym_index == -1) {
                        std::cout << "Error: Symbol not in Symbol Table" << std::endl;
                    }
                ...
                //use semi-colon and flags to find end of expressions
                    ...
                    if (sym_index != -1) {
                        ...
                        gen_instr("PUSHI", std::stoi(inputstring[index + 1].lexeme));
                        skip = true;
                    }
                    gen_instr("LES", 0);
...
int main(int argc, const char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        std::cout << std::endl << "========== Printing Instruction table ==========" << std::endl << std::endl;
        print_instr_table();
        std::cout << std::endl << "========== Printing Symbol table ==========" << std::endl << std::endl;
        print_sym_table();
    }
    return 0;
}